...
func test_inizio_nella_prima_tappa():
	assert_eq(GameManager.stato.viaggio["corrente"], "ciclope")
	assert_eq(GameManager.stato.ulisse["episodio_corrente"], "ciclope")

func test_locale_di_tappa_acceso_e_eleggibile():
	# Polifemo (locale di 'ciclope') deve essere in gioco ed eleggibile nella sua tappa.
...
func test_avanza_per_tetto_turni():
	# Senza azione di progresso, la tappa si chiude al tetto (ciclope: 8).
	var esito := {}
	for i in 8:
		esito = await GameManager.esegui_turno("Osservo l'antro in silenzio.")  # tag []
