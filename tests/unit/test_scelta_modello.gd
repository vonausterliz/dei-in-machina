...
func before_each():
	...
	_modelli_prima = []
	_preferenze_prima = []
	for i in LLMManager.profili_esterni.size():
		_modelli_prima.append(String(LLMManager.profili_esterni[i].get("model", "")))
		_preferenze_prima.append(LLMManager.modello_ricordato(i))

func after_each():
	for i in LLMManager.profili_esterni.size():
		LLMManager.profili_esterni[i]["model"] = _modelli_prima[i]
...
func test_il_modello_si_ricorda_per_ogni_provider():
	if LLMManager.profili_esterni.size() < 2:
		pending("servono almeno due profili"); return
	LLMManager.provider_esterno_idx = 0
	LLMManager.ricorda_modello("modello-del-primo")
	LLMManager.provider_esterno_idx = 1
...
func test_il_modello_torna_nel_profilo_anche_a_motore_spento():
	...
	assert_eq(String(LLMManager.profili_esterni[0]["model"]), "un-modello-scelto",
